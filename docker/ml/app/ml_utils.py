...
class Agent():
    ...
    def train_init(self, events):
        ...
        for event in events:
            ...
            if max_gain is not None:
                ...
                buy_action = 400000000 * buy_action / (1 + 800000000 * abs(buy_action))
                buy_price = max(-1, min(0, min_gain2 * 85))
                sell_price = min(1, max(0, max_gain * 85))
    ...
    def test(self, events):
        ...
        self.score += score + min(1,len(self.portfolio)) / 10000 + self.n_sold / self.n_ticks / 30000
